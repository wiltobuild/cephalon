...
export class AssetService {
  ...
  async resolve(kind: string, name: string): Promise<string | undefined> {
    ...
    if (kind === "shard") {
      // Both tiers use the same crystal art: the manifest's Tauforged entry is a
      // glow-only sprite (ArchonShard*MythicGlow.png) and the wiki's Tauforged
      // render is a glow-dominated hero shot that reads as a blur at UI sizes.
      // The Tauforged tier is conveyed by the "TAU" label and a gold ring in CSS.
      const shard = normalize(name).match(
        /^(?:tauforged )?(amber|azure|crimson|emerald|topaz|violet) archon shard$/,
      );
      if (shard) {
        const color = shard[1][0].toUpperCase() + shard[1].slice(1);
        return `https://wiki.warframe.com/images/${color}ArchonShard.png`;
      }
    }
