...
export type WeaponListItem = { id: string; name: string; category: string; disposition?: number; isIncarnon: boolean };
export type CompatibleMod = { id: string; name: string; polarity: string; rarity: string; maxRank: number; primaryEffect: string };
export type WeaponDetail = { base: Record<string, number>; modSlotCount: number; arcaneSlots: number; incarnon?: { forms: string[] }; mods: CompatibleMod[] };
